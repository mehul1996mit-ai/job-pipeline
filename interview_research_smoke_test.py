...
print("== research_company via fake call_fn (never hits network)")
...
with interview_store.connect() as conn:
    facts = research.research_company(conn, target_id, "TargetBank", call_fn=fake_research_call)
...
print("== ResearchUnavailable — never silently falls back to ungrounded guess")


def failing_call(prompt):
    raise research.ResearchUnavailable("quota exhausted")


raised = False
try:
    with interview_store.connect() as conn:
        research.research_company(conn, target_id, "TargetBank", call_fn=failing_call)
except research.ResearchUnavailable:
    raised = True
check("a failing grounded call raises ResearchUnavailable, not a silent empty result", raised)
...
with interview_store.connect() as conn:
    metrics = research.collect_financial_metrics(
        conn, target_id, "banking_nbfc_housing_finance", "TargetBank", call_fn=fake_metrics_call)
